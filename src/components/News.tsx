  ...
  >
    ...
    <div className="relative h-48 overflow-hidden">
      ...
      <div className="relative z-10 flex h-full flex-col justify-between p-6 text-white">
        <div className="flex items-center justify-between">
          ...
          {event.ceus > 0 && (
            <Badge className="bg-white/20 text-white hover:bg-white/30 backdrop-blur-sm">{event.ceus} CEUs</Badge>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-sm font-medium tracking-wider text-white/90">
              {event.date instanceof Date ? event.date.toLocaleDateString() : event.date}
            </p>
          </div>
          <div className="flex items-center gap-2 mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-white/90">{event.location}</p>
          </div>
        </div>
      </div>
    ...
    <CardContent className="p-6 bg-white">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" className="text-[#003366] hover:text-[#0056b3]">
          Learn More →
        </Button>
      </div>
    </CardContent>
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const [latest, firstTwo] = await Promise.all([getLatestEvent(), getFirstTwoEvents()])
        ...
        if (firstTwo) {
          const filtered = firstTwo.filter((ev) => !latest || ev.id !== latest.id)
      ...
      <section className="w-full bg-gray-50 py-16">
        <div className="container mx-auto px-4 text-center">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-8 max-w-md mx-auto">
            ...
            <h3 className="text-lg font-semibold text-blue-800 mb-2">No Events Available</h3>
            <p className="text-blue-600">Check back soon for upcoming events and announcements.</p>
          </div>
        </div>
      </section>
    )
              ...
              {events.map((event) => (
                ...
                >
                  <div className="flex flex-col md:flex-row">
                    <div className="relative w-[320px] h-[320px] flex-shrink-0 overflow-hidden">
                      <Image
                        src={event.image || "/placeholder.png"}
                        alt={event.title}
                    ...
                    <CardContent className="flex-1 p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        {event.ceus > 0 && <Badge className="bg-blue-100 text-blue-800">{event.ceus} CEUs</Badge>}
                      </div>
                      <p className="text-gray-700 mb-4 line-clamp-2">{event.description}</p>
                      <Button
                        variant="outline"
                        className="text-[#003366] border-[#003366] hover:bg-[#003366] hover:text-white bg-transparent"
                      >
                        Learn More →
                      </Button>
                    </CardContent>
